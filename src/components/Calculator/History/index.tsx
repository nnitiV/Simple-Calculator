...
interface Arguments {
    history: string[]
    goBackToEvaluatedCalculus: (index: number) => void
    clearHistory: () => void
}

export default function History({ history, goBackToEvaluatedCalculus, clearHistory }: Arguments) {
    return (
        <div className={styles.history}>
            <header className={styles.historyHeader}>
                <h1>History:</h1>
                <p onClick={clearHistory}><svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="100" height="100" viewBox="0 0 48 48">
            ...
            {history.length == 0 ?
                <h2>No calculus made for the moment.</h2>
                :
                history.map((item, index) => <div key={index} onClick={() => goBackToEvaluatedCalculus(index)}>{item}</div>)}
        </div>
    )
};